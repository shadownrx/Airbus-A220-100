local simDR_engine1_N2 = find_dataref("sim/flightmodel/engine/ENGN_N2_[0]")
local simDR_engine2_N2 = find_dataref("sim/flightmodel/engine/ENGN_N2_[1]")
local simDR_fuel_flow = find_dataref("sim/flightmodel/engine/ENGN_FF_[0]")
local simDR_fuel_flow2 = find_dataref("sim/flightmodel/engine/ENGN_FF_[1]")
local simDR_apu_bleed_psi = find_dataref("sim/cockpit2/pressurization/actuators/bleed_air_apu")
local simDR_fuel_pump = find_dataref("sim/cockpit2/fuel/fuel_tank_selector")
local simDR_start_switch_1 = find_dataref("sim/cockpit2/engine/actuators/ignition_key[0]")
local simDR_start_switch_2 = find_dataref("sim/cockpit2/engine/actuators/ignition_key[1]")

local simDR_engine1_EGT = find_dataref("sim/cockpit2/engine/indicators/EGT_deg_C[0]")
local simDR_engine2_EGT = find_dataref("sim/cockpit2/engine/indicators/EGT_deg_C[1]")
...
local function update_engine2()
    if not engine2_started then
        ...
        if status == "Engine 2 starting..." then -- TODO: use state machine
